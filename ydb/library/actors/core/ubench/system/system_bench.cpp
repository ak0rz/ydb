...
namespace {
    ...
    struct TBenchResult {
        ...
        void PrintCSV() const {
            ...
            std::printf("basic,%s,%u,%u,%.0f,%.2f,%.2f,%.1f\n",
                        Scenario.c_str(), Threads, ActorPairs,
    ...
    TBenchResult RunPingPong(ui32 threads, ui32 pairs,
                             ui32 warmupSec, ui32 durationSec) {
        auto setup = MakeBasicSetup(threads);
        ...
        for (ui32 i = 0; i < pairs; ++i) {
            ...
            NActors::TActorId pongId = sys.Register(pong, NActors::TMailboxType::HTSwap, 0);

            auto* ping = new TPingActor(pongId, counter, stop);
            NActors::TActorId pingId = sys.Register(ping, NActors::TMailboxType::HTSwap, 0);
    ...
    TBenchResult RunStar(ui32 threads, ui32 senders,
                         ui32 warmupSec, ui32 durationSec,
        ...
        NActors::TActorId receiverId = sys.Register(receiver, NActors::TMailboxType::HTSwap, 0);

        // N senders
        for (ui32 i = 0; i < senders; ++i) {
            auto* sender = new TStarSender(receiverId, stop, &sendToReceiverCount, senderWorkIters);
            NActors::TActorId senderId = sys.Register(sender, NActors::TMailboxType::HTSwap, 0);
            // Kick off
    ...
    TBenchResult RunChain(ui32 threads, ui32 chainLen,
                          ui32 warmupSec, ui32 durationSec) {
        auto setup = MakeBasicSetup(threads);
        NActors::TActorSystem sys(setup);
        sys.Start();
        ...
        for (ui32 i = 0; i < chainLen; ++i) {
            ...
            NActors::TActorId id = sys.Register(actor, NActors::TMailboxType::HTSwap, 0);
            ids.push_back(id);
        }
    ...
    TBenchResult RunReincarnation(ui32 threads, ui32 actors,
                                  ui32 warmupSec, ui32 durationSec) {
        auto setup = MakeBasicSetup(threads);
        NActors::TActorSystem sys(setup);
        sys.Start();
    ...
    TBenchResult RunPipeline(ui32 threads, ui32 pipelines,
        ...
        for (ui32 i = 0; i < pipelines; ++i) {
            ...
            NActors::TActorId nextId = sys.Register(sink, NActors::TMailboxType::HTSwap, 0);

            for (ui32 s = 0; s < stages; ++s) {
                auto* stage = new TPipelineStage(nextId, stageWork);
                nextId = sys.Register(stage, NActors::TMailboxType::HTSwap, 0);
            }

            auto* source = new TPipelineSource(nextId, stop);
            NActors::TActorId sourceId = sys.Register(source, NActors::TMailboxType::HTSwap, 0);
    ...
    TBenchResult RunStorageNode(ui32 threads,
                                const TStorageNodeParams& params,
                                ui32 warmupSec, ui32 durationSec) {
        auto setup = MakeBasicSetup(threads);
        NActors::TActorSystem sys(setup);
        ...
        for (ui32 i = 0; i < params.VDisks; ++i) {
            ...
            auto loggerId = sys.Register(logger, NActors::TMailboxType::HTSwap, 0);

            // Compaction worker for this VDisk
            auto* compaction = new TCompactionActor(params.CompactionWorkIters);
            auto compactionId = sys.Register(compaction, NActors::TMailboxType::HTSwap, 0);

            // VDisk skeleton
            auto* skeleton = new TVDiskSkeleton(
                loggerId, compactionId, counter, stop, params, 0);
            auto skeletonId = sys.Register(skeleton, NActors::TMailboxType::HTSwap, 0);
        ...
        for (ui32 i = 0; i < params.Clients; ++i) {
            ...
            auto clientId = sys.Register(client, NActors::TMailboxType::HTSwap, 0);
...
int main(int argc, const char* argv[]) {
    ...
    opts.SetTitle("Actor system benchmark (basic executor pool)");
    ...
    for (const auto& sc : scenarios) {
        for (ui32 t : threads) {
            if (sc == "storage-node") {
                auto result = RunStorageNode(t, storageParams, warmupSec, durationSec);
                result.PrintCSV();
                std::fflush(stdout);
            } else if (sc == "pipeline") {
                for (ui32 p : pairs) {
                    auto result = RunPipeline(t, p, pipelineStages, stageWorkIters,
                ...
                for (ui32 p : pairs) {
                    TBenchResult result;
                    if (sc == "ping-pong") {
                        result = RunPingPong(t, p, warmupSec, durationSec);
                    } else if (sc == "star") {
                        result = RunStar(t, p, warmupSec, durationSec, senderWork);
                    } else if (sc == "chain") {
                        result = RunChain(t, p, warmupSec, durationSec);
                    } else if (sc == "reincarnation") {
                        result = RunReincarnation(t, p, warmupSec, durationSec);
                    }
                    result.PrintCSV();
                    std::fflush(stdout);
                }
